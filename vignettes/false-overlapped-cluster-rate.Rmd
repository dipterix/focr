...
Let's generate a 2D sample data ($32\times 32$). The red colored pixels are non-zeros, and white pixels are true null pixels. Our goal is to extract non-zero pixels (red).
...
For each pixel $s$, we test on $H_{0}(s): f(s)=0$ versus $H_{1}(s): f(s)\neq 0$. 
Next, we perform the `FOCR-LAWS` procedure in one line.

```{r}
# Run FOCR-BH
dim(data)
res <- focr(data = data, block_size = 11, 
            alpha = 0.05, fdr_method = 'LAWS', 
            dimension = dim(mu), 
            verbose = TRUE)
```

Notice `data` is flattened as a matrix of dimension $100 \times 1024$ (100 observations and $32\times 32$ columns)
...
```{r FOCR-LAWS-2D, fig.width=7, fig.height=6}
par(mfrow = c(2, 2))
...
image(img, zlim = c(0,1), col = pal, 
      main = 'FOCR-LAWS final rejections (proposed)', 
...
# as FOCR-LAWS
rej <- LAWS(pvals, alpha = 0.05, bandwidth = 50, initial_filter = 0.5)
bh <- rej$rejs
img <- matrix(NA, 32, 32)
img[bh] <- 1
image(img, zlim = c(0,1), col = pal, 
      main = 'LAWS rejections as comparison',
      sub = sprintf("FDP: %.1f%%, Power: %.1f%%",
...
pvals <- res$uncond_pvals
dim(pvals) <- dim(mu)
rej <- BH(pvals, alpha = 0.05)
bh <- rej$rejs
img <- matrix(NA, 32, 32)
img[bh] <- 1
image(img, zlim = c(0,1), col = pal, 
      main = 'BH rejections as comparison',
      sub = sprintf("FDP: %.1f%%, Power: %.1f%%",
                    fdp(bh, generator$support)*100,
                    pwr(bh, generator$support)*100))
```
