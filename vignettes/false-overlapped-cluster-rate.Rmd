...
library(focr)
set.seed(400)
generator <- simulation_data_2D(cov_type = 'AR', n_obs = 100, rho = 0.3)
...
res_LAWS <- focr(data = data, block_size = block_size, 
            alpha = 0.05, fdr_method = 'LAWS', bandwidth = 1.5,
            dimension = dim(mu), verbose = TRUE)
res_SABHA <- focr(data = data, block_size = block_size, 
            alpha = 0.05, fdr_method = 'SABHA', bandwidth = 1.5,
            dimension = dim(mu), verbose = TRUE)
...
# as FOCR-LAWS
rej_LAWS <- LAWS(pvals, alpha = 0.05, bandwidth = 1.5, 
                 dimension = 'two', verbose = TRUE)
rej_SABHA <- SABHA(pvals, alpha = 0.05, bandwidth = 1.5, 
                   dimension = 'two', verbose = TRUE)
