...
* Stage-I: given blocks of hypotheses, control the FOCR (an extended false cluster rate) at given level. This step is to find connected regions of interests. The FOCR is defined as: <br /><br />
$\text{FOCR} = \mathbb{E}\left[V  / R \times I_{\{R>0\}}\right],$ <br /><br />
where $V={\sum}_{k}\nu(D_{k})R(B_{k})(1-\theta(B_{k}))$ and $R=\sum_{k}\nu(D_{k})R(B_{k})$. $B_{k}$ are the overlapped clusters (blocks) and $D_{k}$ are the corresponding disjoint kernels;
...
image(mat, main = "FOCR initial rejections (FOCR=0.05)")

image(1-matrix(res$cond_pvals, 32, 32), main = "Conditional p-values")

mat <- matrix(0, 32, 32)
mat[res$post_selection$rejs] <- 1
image(mat, main = "FOCR-BH final rejections (FDR=0.05)")
```

For detailed usage, please read [vignettes](http://github.dipterix.org/focr/articles/false-overlapped-cluster-rate.html), and [help documents](http://github.dipterix.org/focr/reference/focr.html).
